          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-2">
            {missionVision.map((item) => {
              ...
              return (
                <article
                  key={item.title}
                  className="rounded-[2rem] border border-border dark:border-purple-500/25 bg-white/90 dark:bg-[rgba(14,9,28,0.65)] p-8 group hover:border-purple-500/50 dark:hover:border-purple-400/55 hover:bg-white dark:hover:bg-[rgba(20,12,45,0.90)] hover:shadow-[0_8px_32px_rgba(139,92,246,0.10)] dark:hover:shadow-[0_0_50px_rgba(109,40,217,0.32)] transition duration-300 relative overflow-hidden"
                >
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-sm font-bold uppercase tracking-[0.3em] text-purple-600">
                      {item.number}
                    </span>
                    <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-foreground dark:bg-white text-background dark:text-black transition-all duration-300 group-hover:bg-purple-600 group-hover:text-white group-hover:shadow-[0_0_20px_rgba(168,85,247,0.4)]">
                      <Icon className="h-6 w-6" />
                    </div>
                  </div>

                  <h3 className="mt-8 text-2xl font-semibold text-foreground dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-300 transition-colors">{item.title}</h3>
                  <p className="mt-4 text-sm leading-7 text-muted-foreground dark:text-gray-400 sm:text-base">
                    {item.description}
                  </p>
